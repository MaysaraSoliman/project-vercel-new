...
} from "../../queries/dynamicShopQueries";

export default function DynamicShop({ Gender }: { Gender: string }) {
  ...
  const [stateToUseInQuery, setStateToUseInQuery] = useState(
    GET_AllProducts_GENDER
  );
  ...
  useEffect(() => {
    if (searchValue !== "") {
      setStateToUseInQuery(Get_ALLPRODUCTS_GENDER_FilterBySearch);
      setDisableSelectMenu(true);
    } else if (selectedCategory === "All") {
      setStateToUseInQuery(GET_AllProducts_GENDER);
      setDisableSelectMenu(false);
    } else {
      setStateToUseInQuery(GET_AllProducts_GENDER_CategoryFiltered_Sorted);
      setDisableSelectMenu(false);
    }
